...
// Esto sirve para editar catalogo
  ...
  addComentarioDeActividad(title, comment) : Observable<Actividad> {
    const url = `${this.actividadesUrl}/addComment/${title}`;
    return this.http.post<Actividad>(url, comment).pipe(
      tap(_ => this.log('')),
      catchError(this.handleError<Actividad>(`error`)));
  }
